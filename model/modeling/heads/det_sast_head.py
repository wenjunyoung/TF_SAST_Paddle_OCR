...
class tf_SAST_Header1(tf.keras.Model):
    def __init__(self, in_channels, **kwargs):
        ...
        out_channels = [64, 64, 128]
        self.score_conv = tf.keras.Sequential([
            tf_ConvBNLayer(in_channels, out_channels[0], 1, 1, act='relu', name='f_score1'),
            tf_ConvBNLayer(out_channels[0], out_channels[1], 3, 1, act='relu', name='f_score2'),
            tf_ConvBNLayer(out_channels[1], out_channels[2], 1, 1, act='relu', name='f_score3'),
            tf_ConvBNLayer(out_channels[2], 1, 3, 1, act=None, name='f_score4')
        ])
        self.border_conv = tf.keras.Sequential([
            tf_ConvBNLayer(in_channels, out_channels[0], 1, 1, act='relu', name='f_border1'),
            tf_ConvBNLayer(out_channels[0], out_channels[1], 3, 1, act='relu', name='f_border2'),
            tf_ConvBNLayer(out_channels[1], out_channels[2], 1, 1, act='relu', name='f_border3'),
            tf_ConvBNLayer(out_channels[2], 4, 3, 1, act=None, name='f_border4')            
        ])
    ...
    def call(self, x):
        f_score = self.score_conv(x)
        f_score = tf.keras.activations.sigmoid(f_score)
        f_border = self.border_conv(x)
        return f_score, f_border
...
class tf_SAST_Header2(tf.keras.Model):
    def __init__(self, in_channels,  **kwargs):
        ...
        out_channels = [64, 64, 128]
        self.tvo_conv = tf.keras.Sequential([
            tf_ConvBNLayer(in_channels, out_channels[0], 1, 1, act='relu', name='f_tvo1'),
            tf_ConvBNLayer(out_channels[0], out_channels[1], 3, 1, act='relu', name='f_tvo2'),
            tf_ConvBNLayer(out_channels[1], out_channels[2], 1, 1, act='relu', name='f_tvo3'),
            tf_ConvBNLayer(out_channels[2], 8, 3, 1, act=None, name='f_tvo4')
        ])
        self.tco_conv = tf.keras.Sequential([
            tf_ConvBNLayer(in_channels, out_channels[0], 1, 1, act='relu', name='f_tco1'),
            tf_ConvBNLayer(out_channels[0], out_channels[1], 3, 1, act='relu', name='f_tco2'),
            tf_ConvBNLayer(out_channels[1], out_channels[2], 1, 1, act='relu', name='f_tco3'),
            tf_ConvBNLayer(out_channels[2], 2, 3, 1, act=None, name='f_tco4')            
        ])
...
class tf_SASTHead(tf.keras.Model):

    def __init__(self, in_channels,  **kwargs):
        super(tf_SASTHead, self).__init__()

        self.head1 = tf_SAST_Header1(in_channels)
        self.head2 = tf_SAST_Header2(in_channels)
