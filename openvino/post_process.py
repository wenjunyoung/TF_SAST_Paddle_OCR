...
def resize_image_type2(resize_long, img):
        h, w, _ = img.shape
        ...
        if resize_h > resize_w:
            ratio = float(resize_long) / resize_h
        else:
            ratio = float(resize_long) / resize_w
...
class PostProcess(object):
    ...
    def restore_quad(self, tcl_map, tcl_map_thresh, tvo_map):
        """Restore quad."""
        # print("tcl_map:{}, tcl_map_thresh:{}, tvo_map:{}".format(tcl_map.shape, tcl_map_thresh, tvo_map.shape))
        xy_text = np.argwhere(tcl_map[:, :, 0] > tcl_map_thresh)
        # print("xy_text:{}".format(xy_text.shape))
