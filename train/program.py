...
from model.utils.save_load import tf_save_model
...
def check_gpu(use_gpu):
    """
    Log error and exit when set use_gpu=true in Tensorflow
    cpu version.
    """
    err = "Config use_gpu cannot be set as true while you are " \
          "using Tensorflow cpu version ! \nPlease try: \n" \
          "\t1. Install Tensorflow-gpu to run model on GPU \n" \
...
@tf.function
def train_step(model, optimizer, images, loss_class, batch):
    with tf.GradientTape() as tape:
        pres = model(images, training=True)
        loss = loss_class(pres, batch)
    gradients = tape.gradient(loss, model.trainable_variables)
    optimizer.apply_gradients(zip(gradients, model.trainable_variables))
    return loss

@tf.function
def val_step(model, images, batch, post_process_class, use_srn=False):
    ...
    if use_srn:
        ...
        with tf.GradientTape() as tape:
            preds = model(images)
    print("preds type:", type(preds['f_score']))
    print("\n\n preds['f_score'] :", preds['f_score'])
    # print("preds:{}\n\nbatch:{}".format(preds, batch[1]))
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        for idx, batch in enumerate(train_dataloader): 
            ...
            for i in range(len(batch)):
                batch[i] = paddle.to_tensor(batch[i])
...
def tf_train(config,
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        train_dataloader = tf_build_dataloader(config, 'Train', device, logger, seed=epoch)      
        ...
        for idx, batch in enumerate(train_dataloader): 
            ...
            # gradients = tape.gradient(loss, model.trainable_variables)
            # optimizer.apply_gradients(zip(gradients, model.trainable_variables))
            loss = train_step(model, optimizer, images, loss_class, batch)
            ...
            if global_step > start_eval_step and (global_step - start_eval_step) % eval_batch_step == 0 :
                ...
                if vdl_writer is not None:
                    for k, v in cur_metric.items():
                        if isinstance(v, (float, int)):
                            vdl_writer.add_scalar('EVAL/{}'.format(k), cur_metric[k], global_step)
                ...
                # logger best metric
                if vdl_writer is not None:
                    vdl_writer.add_scalar('EVAL/best_{}'.format(main_indicator), best_model_dict[main_indicator], global_step)
...
'''
def eval(model, valid_dataloader, post_process_class, eval_class, use_srn=False):
    model.eval()
    with paddle.no_grad():
        total_frame = 0.0
        total_time = 0.0
        ...
        for idx, batch in enumerate(valid_dataloader):
            ...
            # Obtain usable results from post-processing methods
            post_result = post_process_class(preds, batch[1])
            print("post result type:", type(post_result))
...
def tf_eval(model, valid_dataloader, post_process_class, eval_class, use_srn=False):
    ...
    for idx, batch in enumerate(valid_dataloader):
        ...
        # post_result = val_step(model, images, batch, post_process_class, use_srn)

        if use_srn:
            others = batch[-4:]
            preds = model(images, others, training=False)
        else:
...
def preprocess(is_train=False):
    ...
    use_gpu_id = config['Global']['gpu_id']
    check_gpu(use_gpu)
    ...
    device = 'gpu:0'#.format(use_gpu_id)
...
def tf_preprocess(is_train=False):
    FLAGS = ArgsParser().parse_args()
    config = load_config(FLAGS.config)
    merge_config(FLAGS.opt)
    ...
    if config['Global']['use_visualdl']:
        ...
        vdl_writer = None
    print_dict(config, logger)
    logger.info('train with Tensoflow {} and device {}'.format(tf.__version__, device))
    return config, device, logger, vdl_writer
